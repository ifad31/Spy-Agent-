...
const NO_PFP_URL           = "https://i.imgur.com/0wZQ2zP.png";
...
async function getUserPhotoFileId(userId: number): Promise<string | null> {
  ...
  return res.result.photos[0][0]?.file_id ?? null; // smallest = index 0
}
...
function parseXUrl(text: string): { url: string; username: string; tweetId: string } | null {
  ...
  for (const raw of links) {
    // Strip query params and fragments — keep path only
...
async function handleSubmission(msg: TgMessage, cfg: BotConfig) {
  ...
  // Update user's x_username (exact casing)
  await sbQuery(`users?telegram_id=eq.${from.id}`, {
    method: "PATCH",
    body:   JSON.stringify({ x_username: parsed.username, updated_at: new Date().toISOString() }),
  }).catch(() => {});

  const postNumber  = seasonCount + 1;
  const photoFileId = await getUserPhotoFileId(from.id);
  const photo       = photoFileId ?? NO_PFP_URL;
  const name        = displayName(from);

  const caption  = buildCardCaption(postNumber, name, from.username ?? String(from.id), parsed.url);
